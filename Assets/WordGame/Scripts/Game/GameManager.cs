...
public class GameManager : SingletonComponent<GameManager>
{
    ...
    [Tooltip("The number of hints that a player gets when they first start the game.")] [SerializeField]
    private int startingHints;

    [Tooltip("The GameObject from the Hierarchy that has the LetterBoard component attached to it.")] [SerializeField]
    private LetterBoard letterBoard;

    [Tooltip("The GameObject from the Hierarchy that has the WordGrid component attached to it.")] [SerializeField]
    private WordGrid wordGrid;

    [Tooltip("The prefab from the Project folder that has the LetterTile component attached to it.")] [SerializeField]
    private LetterTile letterTilePrefab;

    [Tooltip("All the categories that are in the game. Levels are assigned to categories.")] [SerializeField]
    private List<CategoryInfo> categoryInfos;

    [Tooltip("A list of levels which will be randomly choosen from when a daily puzzle starts.")] [SerializeField]
    private List<LevelInfo> dailyPuzzles;

    [Tooltip("The sprite that appears in the top bar when a daily puzzle is being played.")] [SerializeField]
    ...
    private void HandleServerScoreUpdate(NetworkManager.ScoreUpdateData scoreData)
    {
        ...
        Debug.Log(
            $"[Server Score] Total: {scoreData.totalScore} | Gained: {scoreData.scoreGained} | Streak: {scoreData.streak}");

        // Show complete overlay with score information
        UIScreenController.Instance.Show(UIScreenController.CompleteScreenId, false, true, false,
            Tween.TweenStyle.EaseOut, null, scoreData);
    }
    ...
    public void DisplayNextHint()
    {
        if (this.CurrentHints == 0)
        {
            ...
            {
                var remainTime = (int)(GameConfig.instance.rewardedVideoPeriod -
                                       CUtils.GetActionDeltaTime("rewarded_video"));
        ...
        else if (this.ActiveBoardState != null && this.CurrentHints > 0)
        {
            ...
            int hintWordIndex;
            int hintLetterIndex;
            var hintDisplayed = this.wordGrid.DisplayNextHint(ref this.ActiveBoardState.nextHintIndex,
                out hintWordIndex, out hintLetterIndex);
    ...
    private void OnWordFound(string word, List<LetterTile> letterTile, bool foundAllWords)
    {
        ...
        {
            this.wordGrid.FoundWord(word, letterTile,
                (GameObject obj, object[] objs) => { this.AnimatingWord = false; });
        }
